...
describe('Auth routes', () => {
  describe('POST /auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      const dbUser = await prisma.Sys_User.findFirst({ where: res.body.user.id });
  ...
  describe('POST /auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      const dbRefreshTokenDoc = await prisma.Sys_Token.findFirst({
  ...
  describe('POST /auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      const dbRefreshTokenDoc = await prisma.Sys_Token.findFirst({ where: { token: res.body.refresh.token } });
      expect(dbRefreshTokenDoc).toMatchObject({ type: tokenTypes.REFRESH, user: userOne._id, blacklisted: false });

      const dbRefreshTokenCount = await prisma.Sys_Token.count();
  ...
  describe('POST /auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      const dbResetPasswordTokenDoc = await prisma.Sys_Token.findFirst({
        where: { token: resetPasswordToken, user: userOne._id },
      });
  ...
  describe('POST /auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const dbUser = await prisma.Sys_User.findFirst({ where: { userid: userOne._id } });
      const isPasswordMatch = await bcrypt.compare('password2', dbUser.password);
      expect(isPasswordMatch).toBe(true);

      const dbResetPasswordTokenCount = await prisma.Sys_Token.count({
        where: { user: userOne._id, type: tokenTypes.RESET_PASSWORD },
      });
  ...
  describe('POST /auth/send-verification-email', () => {
    ...
    test('should return 204 and send verification email to the user', async () => {
      ...
      const dbVerifyEmailToken = await prisma.Sys_Token.findFirst({ where: { token: verifyEmailToken, user: userOne._id } });
  ...
  describe('POST /auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      const dbVerifyEmailToken = await prisma.Sys_Token.count({
        where: {
          user: userOne._id,
