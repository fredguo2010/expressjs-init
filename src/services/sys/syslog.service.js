 ...
 */
const getall = async () => {
  return prisma.Sys_Log.findMany();
};

/**
 * pagination for sysLogs
...
const pagination = async (filter, options) => {
  ...
  const count = await prisma.Sys_Log.count({ where: filter });

  const sysLogs = await prisma.Sys_Log.findMany({
    where: filter,
    skip: (page - 1) * row,
    take: row,
    orderBy: orderByObj,
  });
  return { isok: true, total: count, data: sysLogs };
};
...
const getsysLogById = async (cGuid) => {
  return prisma.Sys_Log.findUnique({
    where: {
      cGuid,
    },
  });
...
const addsysLog = async (createBody) => {
  const sysLog = await prisma.Sys_Log.create({
    data: createBody,
  });
  return sysLog;
};
...
const updatebyid = async (cGuid, updateBody) => {
  ...
  await prisma.Sys_Log.updateMany({
    data: updateBody,
    where: {
      cGuid,
    },
